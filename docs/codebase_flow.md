...
*   [`models/response.py`](../models/response.py): Defines the final `Response` payload containing `answer`, `source`, `confidence`, and `citations`. Houses the public `NOT_FOUND_MESSAGE` fallback refusal constant.
*   [`models/collection.py`](../models/collection.py): Stores document collection properties (`name`, `path`, `documents`).
*   [`models/retrieval_candidate.py`](../models/retrieval_candidate.py): Standardizes evidence segments across both PDF distance matrices and web search relevance outputs.
*   [`models/tool_call.py`](../models/tool_call.py) / [`models/tool_result.py`](../models/tool_result.py): Manages agent tool invocation state.
...
*   [`services/crew_service.py`](../services/crew_service.py): Orchestrates CrewAI agents. Defines specific constraints (`max_iter`, output character limits) to safeguard operations against rate limits.
*   [`services/memory_service.py`](../services/memory_service.py): Manages SQLite memory initialization (`db/memory.db`) and SQL queries for logging reports and editing preferences.
*   [`services/mcp_client.py`](../services/mcp_client.py): Spawns JSON-RPC stdio subprocesses to negotiate dynamic tool bindings.

#### RAG & Retrieval Layer:
*   [`services/conversation.py`](../services/conversation.py): Coordinates the RAG question routing and message memory.
*   [`services/question_rewriter.py`](../services/question_rewriter.py): Uses the LLM to rewrite contextual queries into standalone search questions.
*   [`services/agent.py`](../services/agent.py): Drives the single-agent ReAct planning loop.
*   [`services/retriever.py`](../services/retriever.py): Performs similarity searches inside local Chroma DB.
*   [`services/web_search.py`](../services/web_search.py): Queries the Tavily Search API with advanced options.
*   **Ranking & Evaluation:**
    *   [`services/reranker.py`](../services/reranker.py): Leverages Hugging Face embeddings to compute similarity cosine distances for cross-source sorting.
    *   [`services/evaluator.py`](../services/evaluator.py): Scores candidate relevance to assign Confidence values.

---

### 3. `config/` (Parameters)
*   [`config/settings.py`](../config/settings.py): Centralizes all configurations, including document chunk sizes, relevance thresholds, models, and AI gateway settings.
