...
#### Option A: Windows (Recommended via `uv`)
> 💡 **Recommended for Windows:** Standard `pip` may enter a dependency backtracking loop between `crewai` and `litellm`, eventually attempting to build `litellm` from source and failing with `Cargo, the Rust package manager, is not installed`. 
> 
> Installing **`uv`** avoids this completely—it resolves all 174 dependencies in under 1 second and downloads pre-built binary wheels without requiring Rust/Cargo:
...
#### Option B: macOS / Linux (or Standard pip)
```bash
pip install --upgrade pip
pip install -r requirements.txt
```

*(Note: The initial download includes PyTorch, Transformers, and Hugging Face embedding weights, taking ~1-2 minutes depending on your network connection).*
...
│   └── mcp_servers.json        # MCP stdio tool bindings (configured with portable ./data)
...
### Q7: Windows `pip install` fails with "Cargo, the Rust package manager, is not installed"
* **Explanation:** Standard `pip`'s backtracking resolver may try older source distributions of `litellm` (`.tar.gz`) which require the Rust compiler and Cargo to build.
* **Fix:** Use `uv` for package installation. `uv` avoids source-build loops and installs pre-compiled Windows wheels directly:
  ```powershell
  pip install uv
  uv pip install -r requirements.txt
  ```
...
### Q8: Why did my question route to Web Search instead of local PDF?
* **Explanation:** The agent's intelligent router checks the user question against the **Active Document Collection** selected in the main menu:
  - If the active collection is **`medical`** (containing pulmonary rehabilitation & anatomy papers) and you ask *"What is self-attention?"* or *"What is BERT?"*, the router detects that these topics are outside the medical documents and routes to live web search (`search_web`).
  - To query topics from local PDFs, make sure the corresponding collection is active. For machine learning papers (Self-Attention, Transformer, BERT), switch to the **`research`** collection (**Option 5** in `app.py`).

---

### Q9: Cross-Platform Paths and MCP Server Configuration
* **Explanation:** All project paths in `config/mcp_servers.json` and documentation use clean relative paths (`./data`). 
* **Details:** `services/mcp_client.py` dynamically resolves `./data` to the host machine's absolute path at runtime, ensuring full out-of-the-box compatibility on Windows, macOS, and Linux without needing to edit machine-specific directory paths.
